...
import { addContent, getCollectedUrls } from '../storage'
import { searchContents, judgeUrl } from '../claudeApi'
import ContentCard from '../components/ContentCard'

export default function ListingPage({
  groqKey, serperKey, selectedWeek, contents, setContents,
  ...
  const filtered = contents.filter(c => c.category === activeTab && c.weekId === selectedWeek)
  ...
  async function handleSearch() {
    if (!groqKey || !serperKey) { setError('Groq API Key와 Serper API Key를 모두 입력해주세요.'); return }
    ...
    try {
      ...
      for (const item of withWeek) {
        await dbAddContent(item)
        addContent(item)
      }
  ...
  async function handleAddUrl() {
    ...
    try {
      ...
      await dbAddContent(withWeek)
      addContent(withWeek)
      setContents(prev => [withWeek, ...prev])
      setManualUrl('')
    } catch (e) {
      setError(e.message || 'URL 분석 중 오류가 발생했습니다.')
    } finally {
      setAddingUrl(false)
    }
  }
  ...
  return (
    <div>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-0.5 bg-gray-100 rounded p-0.5">
              <button onClick={() => handleTabViewMode('detail')} className={`text-xs px-2 py-0.5 rounded ${currentViewMode === 'detail' ? 'bg-white shadow text-blue-600' : 'text-gray-400'}`} title="상세형">☰</button>
              <button onClick={() => handleTabViewMode('compact')} className={`text-xs px-2 py-0.5 rounded ${currentViewMode === 'compact' ? 'bg-white shadow text-blue-600' : 'text-gray-400'}`} title="간략형">≡</button>
            </div>
          </div>
        </div>

        <button onClick={handleSearch} disabled={loading}
          className="w-full bg-blue-500 hover:bg-blue-600 disabled:bg-blue-300 text-white py-2 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2">
        ...
        {activeTab === 'job' && (
          <div className="mt-3 flex gap-2">
            <input type="url" value={manualUrl} onChange={e => setManualUrl(e.target.value)}
              ...
              placeholder="채용공고 URL 직접 입력..."
              className="flex-1 text-sm border border-gray-300 rounded px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-400" />
            <button onClick={handleAddUrl} disabled={addingUrl || !manualUrl.trim()}
              className="bg-gray-600 hover:bg-gray-700 disabled:bg-gray-300 text-white text-sm px-3 py-1.5 rounded whitespace-nowrap">
      ...
      {filtered.length === 0 ? (
        <div className="text-center text-gray-400 py-12 text-sm whitespace-pre-line">{'아직 수집된 소재가 없습니다.\nAI 검색 버튼을 눌러 소재를 수집하세요.'}</div>
