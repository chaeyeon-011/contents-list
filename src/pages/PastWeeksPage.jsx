        ...
        {CATEGORIES.map(cat => {
          ...
          return (
            <button key={cat.id} onClick={() => { setActiveTab(cat.id); setError(''); setInfo('') }}
              className={`flex-1 text-sm py-2 px-2 rounded-md transition-colors ${activeTab === cat.id ? 'bg-white shadow text-blue-600 font-semibold' : 'text-gray-600'}`}>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <div className={`text-xs px-2 py-1 rounded-full ${okCount >= quota ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{okCount >= quota ? '목표 달성' : `${quota - okCount}개 남음`}</div>
